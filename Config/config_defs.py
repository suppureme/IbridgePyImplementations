...
class UserConfigBase(Printable):  # should not be used directly
    ...
    def validate(self):
        # if hasattr(self, 'dataProviderName'):
        #     exit(__name__ + '::validate: userConfig should not have this value %s' % ('dataProviderName',))
        if self.projectConfig.liveOrBacktest == LiveBacktest.BACKTEST:
            if self.traderConfig.runMode != TraderRunMode.RUN_LIKE_QUANTOPIAN:
                ...
                print(__name__ + '::validate: IBridgePy has reset the TRADER.runMode to RUN_LIKE_QUANTOPIAN for backtesting! Otherwise, backtester will not generate BalanceLog.')
            if self.projectConfig.repBarFreq != 60:
                self.projectConfig.repBarFreq = 60
                print(__name__ + '::validate: IBridgePy has reset projectConfig.repBarFreq to 60 for backtesting!')
    ...
    def prepare_userConfig_with_trader(self, trader, globalsV):
        ...
        # they will by applied to userConfig here
        ...
        if not self.projectConfig.accountCode:
            if self.projectConfig.brokerName == BrokerName.TD:
                ...
                print('User do not specify an accountCode. IBridgePy uses %s from settings.py::BROKER_CLIENT::TD_CLIENT::accountCode' % (accountCode,))
                ...
                print('User do not specify an accountCode. IBridgePy uses %s from settings.py::BROKER_CLIENT::IB_CLIENT::accountCode' % (accountCode,))
                self.projectConfig.accountCode = accountCode
            elif self.projectConfig.brokerName == BrokerName.ROBINHOOD:
                accountCode = self.brokerClientConfig.ROBINHOOD_CLIENT['accountCode']
                print('User do not specify an accountCode. IBridgePy uses %s from settings.py::BROKER_CLIENT::ROBINHOOD_CLIENT::accountCode' % (accountCode,))
                self.projectConfig.accountCode = accountCode
            else:
